...
function StoreService() {
  ...
  this.deepClone = (obj, freeze = false) => {
  ...
  this.getKeyPathsChanged = (obj1, obj2) => {
  ...
  this.saveHistory = (action, changeType) => {
    const newHistoryObj = {}
  ...
  this.combineReducers = (reducerObj) => {
    // Saving reducer.
  ...
  this.dispatch = (action) => {
    ...
    // Need 'KEYPATHS_TO_CHANGE' property in action object if making state changes.
    ...
    for (let keyPath in keyPathsChanged) {
      if (keyPath in this.partialListeners) {
        ...
        this.partialListeners[keyPath].forEach(listener => {
          ++this.listeners
          listener()
        })
      }
  ...
  this.subscribe = (fn, keyPath = null) => {
